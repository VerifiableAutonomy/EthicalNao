...
class Robot:
    ...
    def is_moving(self):
        return self.motion_process.is_alive() or self.rotation_process.is_alive()
    ...
    def point_direction_world(self, world_x, world_y):
        ...
        robot_x = result[0]
        robot_y = result[1]
        if robot_x < 0.1: robot_x = 0.1
        self.point_direction_robot(robot_x, robot_y)

    def point_direction_robot(self, robot_x, robot_y):
    ...
    def start_listen(self, words, wordspotting=False):
        self.logger.write('Start Listening for words: ' + str(words))
        self.stop_listen()
        self.recognition.setLanguage("English")
        self.recognition.setVocabulary(words, wordspotting)
    ...
    def execute_motion(self, desired_x, desired_y, obstacle_avoidance=False, frequency=2, delay=0):
        ...
        while True:
            ...
            start = time.time()
            self.localize_self(print_message=True, head_yaw=head_azimuth)
            ...
            if abs(movement_rotation) > 30: movement_vector*=0
