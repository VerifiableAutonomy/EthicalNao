...
class Webcam:
    ...
    def fetch(self, save_to=None):
        ...
        start = time.time()
        source = urllib.urlopen(self.address)
        data = source.read()
    ...
    def capture(self, folder, rate=1, prefix=''):
        ...
        while True:
            ...
            start = time.time()
            counter += 1
            file_name = prefix + str(counter).zfill(3) + '.jpg'
    ...
    def start_capture(self, folder, rate=1, prefix=''):
        self.stop_capture_process = False
        self.capture_process = threading.Thread(target=self.capture, args=(folder, rate, prefix))
